...
class FoodTruckAPITests: XCTestCase {
  ...
  func testDeleteTruck() {
    ...
    trucks.addTruck(name: "testDelete", foodType: "testDelete", avgCost: 0, latitude: 0, longitude: 0) { (addedTruck, err) in
      ...
      if let addedTruck = addedTruck {
        ...
        // Second add a review
        trucks.addReview(truckId: addedTruck.docId, reviewTitle: "testDelete", reviewText: "testDelete", starRating: 0, completion: { (addedReview, err) in
          guard err == nil else {
            XCTFail()
            return
          }
  ...
  func testGetReviewsForTruck() {
    ...
    trucks.addTruck(name: "testReviewsForTruck", foodType: "testReviewsForTruck", avgCost: 0, latitude: 0, longitude: 0) { (addedTruck, err) in
      ...
      if let addedTruck = addedTruck {
        // Add a review
        trucks.addReview(truckId: addedTruck.docId, reviewTitle: "testReview1", reviewText: "testReview1", starRating: 0, completion: { (review, err) in
          guard err == nil else {
            XCTFail()
            return
          }
          ...
          // Add a second review
            trucks.addReview(truckId: addedTruck.docId, reviewTitle: "testReview2", reviewText: "testReview2", starRating: 0, completion: { (review, err) in
            guard err == nil else {
              XCTFail()
              return
            }
  ...
  func testGetReviewById() {
    ...
    trucks.addReview(truckId: "123456789", reviewTitle: "test", reviewText: "test", starRating: 0) { (review, err) in
      guard err == nil else {
        XCTFail()
        return
      }
  ...
  func testUpdateReview() {
    ...
    // Create a review
    trucks.addReview(truckId: "12345", reviewTitle: "testReview", reviewText: "testReview", starRating: 0) { (addedReview, err) in
      guard err == nil else {
        XCTFail()
        return
      }
  ...
  func testCountAllReviews() {
    ...
    trucks.addReview(truckId: "12345", reviewTitle: "testReview", reviewText: "testReview", starRating: 0) { (review1, err) in
      guard err == nil else {
        XCTFail()
        return
      }
        trucks.addReview(truckId: "34567", reviewTitle: "testReview", reviewText: "testReview", starRating: 0, completion: { (review2, err) in
        guard err == nil else {
          XCTFail()
          return
        }
  ...
  func testCountReviewsForTruck() {
    ...
    trucks.addReview(truckId: "12345", reviewTitle: "testReview", reviewText: "testReview", starRating: 0) { (review1, err) in
      ...
      // add 2nd review
        trucks.addReview(truckId: "12345", reviewTitle: "testReview2", reviewText: "testReview2", starRating: 0) { (review2, err) in
        guard err == nil else {
          XCTFail()
          return
        }
  ...
  func testGetAverageStarRating() {
    ...
    trucks.addReview(truckId: "23456", reviewTitle: "testReview", reviewText: "testReview", starRating: 5) { (review, err) in
      guard err == nil else {
        XCTFail()
        return
      }
        trucks.addReview(truckId: "23456", reviewTitle: "testReview", reviewText: "testReview", starRating: 2, completion: { (review, err) in
        guard err == nil else {
          XCTFail()
          return
        }
        
        trucks.avgStarRating(truckId: "23456", completion: { (rating, err) in
          guard err == nil else {
            XCTFail()
            return
          }
          ...
          // should be rounded to 3
            trucks.addReview(truckId: "23456", reviewTitle: "testReview", reviewText: "testReview", starRating: 3, completion: { (review, err) in
            guard err == nil else {
              XCTFail()
              return
            }
